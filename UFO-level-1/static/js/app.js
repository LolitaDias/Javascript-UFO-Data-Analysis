...
button.on("click", () => {
    ...
    var filteredData=ufo_master_data.filter(ufo => ufo.datetime === inputDateValue);
    console.log(filteredData);
    //Reset the UFO data before displaying the filtered data
    tbody.html("");
    populatedata(filteredData);
    // If Filter Table is clicked without inserting a date
    if(filteredData.length===0){
        tbody.append("tr").append("td").text("There are no UFO Sightings logged for the requested Date!!!"); 
    }
